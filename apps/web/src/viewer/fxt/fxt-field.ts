/**
 * FXT field — additive red-orange markers on the celestial sphere.
 *
 * Same shader skeleton as AtelField; only the colour and the sphere
 * radius differ so the four live-alert layers stack without z-fighting:
 * ATel @ 0.9955, FXT @ 0.9950, GOTO @ 0.9945, BlackGEM @ 0.9940.
 */
...
const VERT = /* glsl */ `
  ...
  void main() {
    float omega = 6.2831853 / 1.3;
    float shimmer = sin(omega * (uTime + aPhase * 1.5));
    float baseScale = 1.0 + 0.22 * (shimmer + 1.0);
...
const FRAG = /* glsl */ `
  ...
  void main() {
    ...
    a *= 0.6 + 0.4 * vPulse;
    // FXT red-orange (hotter than ZTF orange).
    vec3 col = vec3(1.0, 0.36, 0.18);
