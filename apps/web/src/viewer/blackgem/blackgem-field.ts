/**
 * BlackGEM field — additive magenta markers on the celestial sphere.
 *
 * Same shader skeleton as the other live-alert fields; magenta is the
 * 4th colour in the live-alerts palette (yellow=ATel, red-orange=FXT,
 * cyan=GOTO, magenta=BlackGEM). Sphere radius 0.9940 — the deepest
 * shell in the live-alerts stack so overlapping markers never z-fight.
 */
...
import type { BlackGemEvent } from "./feed";

const RADIUS = 0.9940;
const RECENT_PULSE_MS = 1000;

export class BlackGemField {
  readonly group = new Group();
  private points: Points | null = null;
  private items: BlackGemEvent[] = [];
  private material: ShaderMaterial | null = null;
  private appearedAt = new Float32Array(0);
  private startTimeMs = performance.now();

  constructor() {
    this.group.name = "BlackGemField";
  ...
  setData(items: BlackGemEvent[]): void {
...
const VERT = /* glsl */ `
  ...
  void main() {
    float omega = 6.2831853 / 2.1;
    float shimmer = sin(omega * (uTime + aPhase * 1.5));
    float baseScale = 1.0 + 0.19 * (shimmer + 1.0);
