...
export function LeftRail({ state, scene, onOpenGuide, onOpenTimeMachine }: Props) {
  ...
  return (
    <div className="pointer-events-auto absolute left-3 top-32 bottom-32 z-10 flex w-64 flex-col rounded-xl border border-white/10 bg-space-950/85 backdrop-blur">
      ...
      {/* Always-visible AstroGrid-style sliders. Caps the panel before
          the footer-action row so they stay reachable even when the
          Celestial Objects section is scrolled. */}
      <div className="border-t border-white/5 px-3 py-2 space-y-2">
        <Slider
          label="Orbit line opacity"
          value={settings.orbitOpacity}
          min={0}
          max={1}
          step={0.01}
          onChange={(v) => updateSettings({ orbitOpacity: v })}
        ...
        <Slider
          ...
          max={1}
          step={0.01}
          onChange={(v) => updateSettings({ gridOpacity: v })}
          format={(v) => `${Math.round(v * 100)}%`}
        />
        <Slider
          ...
          max={2}
          step={0.01}
          onChange={(v) => updateSettings({ starBrightness: v })}
          format={(v) => `${Math.round(v * 100)}%`}
        />
        <Slider
          label="Fly-to duration"
          value={settings.flyToDurationSec}
          min={1}
          max={10}
          step={0.5}
  ...
  return (
    <div>
      ...
      <input
        ...
        className="mt-0.5 h-1 w-full accent-emerald-400"
        aria-label={label}
      />
    </div>
  );
